...
async function signUp({
  ...
  if (!isExist) {
    console.log('[signUp] 가입되지 않은 회원입니다.'); //회원 가입후 토큰 발급
...
router.get('/callback/:coperation', async (req, res) => {
  try {
    ...
    if (!tokenInfo) return;
    if (coperation === 'google') options.userInfoUrl = `https://www.googleapis.com/oauth2/v2/userinfo?access_token=${tokenInfo.access_token}`;
    const userInfo = await getUserInfo(options.userInfoMethod, options.userInfoUrl, tokenInfo.access_token); // console.log('userInfo = ', userInfo);
    //로그인 되었 을 경우
    ...
    if (userInfo) {
      if (coperation === 'naver') {
        ...
        if (resultcode === '00' && reason === 'success') {
          ...
          let jwtToken = null;
          let refreshToken = null; //회원 가입 안되어있을시
          ...
          const makedResponse = returnResponse({
            res,
            jwtToken,
            refreshToken
          });
      ...
      if (coperation === 'kakao') {
        ...
        if (id && kakao_account) {
          console.log('[kakao] id, kakao_account 존재합니다.'); //1. 회원가입 여부 확인
          ...
          const memberRow = await authDaoNew.getEmailIsAlreadyExist(email, has_email, 'kakao', id);
...
router.get('/login', async (req, res) => {
  ...
  let refreshToken = null;
  const id = 'testid';
  const email = 'testemail'; //1. 회원가입 여부 확인

  console.log('로그인 진입!');
  const memberRow = await authDaoNew.getEmailIsAlreadyExist(email);
  const {
    EXISTFLAG
  } = memberRow[0]; //회원 가입 안되어있을시
