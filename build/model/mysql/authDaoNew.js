...
const setMemberSignUp = async parameter => {
  ...
  try {
    ...
    let [memberInfo] = await connection.query(signUpSql, [mem_username, mem_userid, mem_email, mem_social, mem_nickname]);
...
const getEmailIsAlreadyExist = async (mem_email, has_email = true, social = '', mem_id = '') => {
  let findExistSql = `
  ...
  try {
    //await connection.beginTransaction(); // START TRANSACTION
    let memberInfo;
    if (kakaoHasNotEmail) memberInfo = await connection.query(findExistSql, [mem_id, social]);else memberInfo = await connection.query(findExistSql, [mem_email]);
    await connection.commit(); // COMMIT
...
const setRefreshData = async parameter => {
  ...
  try {
    //await connection.beginTransaction(); // START TRANSACTION
    let [refreshData] = await connection.query(refreshSql, [id, refreshToken, refreshToken]);
    await connection.commit(); // COMMIT
