...
const accessTokenVerify = req => {
  if (req?.headers?.authorization) {
    const accessToken = req.headers.authorization.split('Bearer ')[1]; // access token 검증 -> expired여야 함.

    const authResult = verify(accessToken); // 디코딩 결과가 없으면 권한이 없음을 응답.
...
const refreshTokenVerify = refreshToken => {
  if (refreshToken) {
    ...
    if (!refreshTokenVerified.ok) return {
      ok: false,
    ...
    if (refreshTokenVerified.ok === false && refreshTokenVerified.reason === 'jwt expired') {
      return {
        ok: false,
...
const authorization = async () => {
  ...
  if (accessTokenVerifyInfo.ok && accessTokenVerifyInfo.reason === 'Access token is not expired' && accessTokenVerifyInfo.decoded) {
    return {
      ok: true,
  ...
  if (!accessTokenVerifyInfo.ok && (accessTokenVerifyInfo.reason === 'Access token is expired' || accessTokenVerifyInfo.reason === 'Not have Access token')) {
    return {
      ok: false,
...
const refresh = async (req, res) => {
  ...
  try {
    ...
    if (!accessTokenVerifyInfo.ok && accessTokenVerifyInfo.reason === 'No authorized') {
    ...
    if (!accessTokenVerifyInfo.ok && (accessTokenVerifyInfo.reason === 'Access token is expired' || accessTokenVerifyInfo.reason === 'Not have Access token')) {
      ...
      const refreshToken = cookie.refresh_token; //1. refresh Token이 있는지
      //2. 복호화가 되는지

      const refreshTokenVerifyInfo = refreshTokenVerify(refreshToken, accessTokenVerifyInfo);
      console.log('[refresh] refreshTokenVerifyInfo =', refreshTokenVerifyInfo); // 만약 리프레시 토큰이 expired 됬다면

      if (!refreshTokenVerifyInfo.ok && !refreshTokenVerifyInfo.reason === 'Not have Refresh token') {
        console.log('[refresh] 리프레시 토큰이 없습니다. ');
        return res.status(403).json({
      ...
      if (!refreshTokenVerifyInfo.ok && !refreshTokenVerifyInfo.reason === 'refresh token is expired') {
      ...
      console.log('[refresh] 리프레시 데이터 베이스 조회 ', refreshVerifyDatabaseInfo);
      const loginInfo = refreshVerifyDatabaseInfo.data[0].ID.split('_');
      const id = loginInfo[0];
      const email = loginInfo[1]; // 데이터를 기반으로 accesstoken 발급
      ...
      console.log('[refresh] 리프레시 검증 완료 및 엑세스 토큰 재발급 = ', newAccessToken);
      return res.json({
