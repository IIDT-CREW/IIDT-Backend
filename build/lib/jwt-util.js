require('dotenv').config(); // const { promisify } = require('util');


const jwt = require('jsonwebtoken');

const authDaoNew = require('../model/mysql/authDaoNew'); // const redisClient = require('./redis');
...
module.exports = {
  sign: payload => {
    ...
    return jwt.sign(payload, secret, {
      ...
      algorithm: 'HS256',
      // 암호화 알고리즘
      expiresIn: '4h' //expiresIn: '4h', // 유효기간
  ...
  refreshVerifyDatabase: async (refreshToken, userId) => {
    try {
      ...
      if (userId) {
        data = await authDaoNew.getRefreshData({
          id: userId
        });
      ...
      if (!userId) {
        data = await authDaoNew.getRefreshDataToRefresh({
          refreshToken
        });
